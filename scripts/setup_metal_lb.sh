...
prefix=$(docker inspect kind|jq -r '.[]|select(.Name=="kind").IPAM.Config[]|select(.Gateway|contains(".")).Gateway'|cut -d. -f1-2)
...
KUBECONFIG="$1" kubectl apply -f ${REPO_ROOT}/metal_lb_addrpool.yaml
